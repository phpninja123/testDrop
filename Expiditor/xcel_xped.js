...
console.log('RENAME ---> ' + __dirname + '/done/' + path.basename(filename));
    fs.rename(filename, __dirname + '/done/' + path.basename(filename),function(err){
      ...
      SendAlertMail("Expediting Import Complete","wynand@elecdesign.co.za",filename,__dirname + '/done/' + path.basename(filename))
    }); 
...
function SendAlertMail(message,Escalation_Email_Addr,name,filename)
{
 var sendgrid  = require('sendgrid')('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');
...
email.addCc('wynandvwyk@gmail.com');
email.setFrom('exp_imports@freakydesign.co.za');
email.setSubject("Email processed : " + name);
